...
const {meetingRequestTemplate , meetingRequestAcceptanceTemplate} = require('../tools/meetingRequestEmail');
...
module.exports.addMeetingRequest = async function (req, res) {
  try {
    ...
    const intern = await User.findById(body.intern);
    if (!intern) {
    ...
    const html = meetingRequestTemplate({
    ...
    await transporter.sendMail({
      from: `"InternLink" <${process.env.EMAIL}>`,
      to: body.mentorEmail,
...
module.exports.AcceptMeeting = async function (req, res, next) {
    ...
    try {
        ...
        if (eventExist.length > 0) {
            ...
            const html = meetingRequestAcceptanceTemplate({
                mentorName: req.body.obj.mentorName,
                internName: req.body.obj.internName,
                title: req.body.obj.title,
                note: req.body.obj.note,
